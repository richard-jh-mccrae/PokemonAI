"""Accessors over the raw observation — this option's Pokémon, my Active, an `AttackStat`, the damage
...
class ViewsMixin:
    ...
    def _option_pokemon(self, obs: dict, select: dict, option: dict) -> dict | None:
        """The board card/Pokémon dict an option's (area, index, playerIndex) points at, or None. A PLAY
        option carries a bare hand `index` and no `area`; a DECK option's candidates live on the select."""
        ...
        if area == _LOOKING:                               # a face-up reveal: candidates live in
            looking = state.get("looking") or []           # current.looking, not in a player zone
    ...
    def _my_bench_raws(self, obs: dict) -> list:
        """MY benched bodies' raw dicts — off the SNAPSHOT when one exists, so the bench cannot shift
        under a memoized clock (ADR-0068); off the observation otherwise."""
    ...
    def _stat_of(self, card_id):
        """This card's ``CardStat``, or None — the provider as a CALLABLE, the shape `common.retreat_cost`
        takes so a `StateModel` can supply the same card knowledge the Pilot does."""
    ...
    def _predicted_max_damage(self, attacker_stat, defender: dict | None, *,
        ...
        """The worst damage `attacker_stat` deals to `defender` (Incoming's magnitude), handed the
        per-decision OPPONENT context `_board` stashes."""
    ...
    def _damage_context(self, obs: dict, *, attacker_is_me: bool = True) -> dict:
        """ATTACKER-relative visible-state counts for the Damage Formula's scaling term (ADR-0032). An
        ADAPTER: `damage_context` assembles, and the sides are rebuilt because `_board` precedes the snapshot."""
        state = obs.get("current") or {}
        players = state.get("players") or []
        yi = state.get("yourIndex", 0)
        me = players[yi] if 0 <= yi < len(players) and players[yi] else {}
        ...
        # `own_prizes` is the ONE anchor MySide needs for the exact deck facts (only MY deck can be
        # exact); the model derives the pair from it (`MySide._deck_facts`, Issue #297).
    ...
    def _my_damage_context(self, obs: dict) -> dict:
        """MY-attacker Damage Formula context for THIS decision, built at most once. Keyed by obs IDENTITY,
        but `_turn_boosts` is also read — anything that observes MID-decision must invalidate here too."""
    ...
    def _discard_energy_counts(self, discard: list) -> tuple[int, dict]:
        """Discard-pile Energy histograms: ``(all Energy cards, {energyType: Basic-Energy count})``.
        Unknown cards count 0."""
        total, by_type = 0, {}
        for c in discard:
            cid = (c or {}).get("id")
