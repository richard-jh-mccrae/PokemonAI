...
differencing. `apply_option` says what the board would become; `state_value` says what that is worth;
this puts them in a loop and reports which first action the best sequence commits to.

    score(sequence) = state_value(end board) + EV(terminal action),   EV(end-turn) = 0

ARMED as the MAIN decider (ADR-0131): `planner._composer_line` calls :func:`compose`, deciding 63.4% of
MAIN corpus frames. It ABSTAINS on a tie rather than overrule the sequencer — a composer is a function
of the end state, so some orderings are definitionally out of its reach.

Every ruling this module implements is recorded, and none of it is restated here: deferred-target
EXPANSION in ADR-0121, reveal-truncated lines in ADR-0129, the tie-defer and the
information-before-commitment boundary in ADR-0131 and ADR-0095 decision 3, the differencing track
itself in ADR-0092. What the beam is STRUCTURALLY BLIND TO is a 36-entry checklist in
`data/leaf_lab/wave3-rulings.md` §3b — cited, never copied. `BEAM_WIDTH` / `SEQUENCE_DEPTH` / `EPSILON`
are whitelisted under `sound_rules.composer-budget-caps`, with the derivation on that entry."""
