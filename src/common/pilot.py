...
class Pilot(PlannerMixin, ObjectivesMixin, GustMixin, FetchMixin, ShuffleRefreshMixin, ToolMixin):
    ...
    def __init__(self, strategy, deck, *, general_strategy=None, overrides=None, stats=None,
        ...
        self.deny_relevance = deny_relevance            # ADR-0079 / #199 COMPUTE-ONLY switch: emits
                                                        ...
                                                        # yet (#187 is the consumer), so ON changes
    ...
    def _opponent_target_rows(self, obs: dict, board) -> tuple | None:
        ...
        # Deny Relevance's REDUNDANCY gate (ADR-0079 step 2), resolved once for the whole decision
        ...
        for i, b in enumerate(bodies):
            ...
            if self.deny_relevance:
                row.update(self._relevance_terms(b, ma, doomed=doomed_ids, area=area, bi=bi))
            rows.append(row)
        return phase, rows
    ...
    def _bench_doomed_by_me(self, ma: dict | None, bench_list) -> frozenset:
        ...
        The bench half of the Deny Relevance redundancy gate (ADR-0079 decision 1, step 2): *"or
    ...
    def _relevance_terms(self, b, ma, *, doomed: frozenset, area: str, bi: int) -> dict:
        """The DENY instrument's value — **Deny Relevance**, the read that replaced the magnitude
        (ADR-0079, #199 grill; `common/deny_relevance.py` owns the scoring, this owns the plumbing).
        ...
        — nothing reads these yet, #187 is the consumer (ADR-0079 decision 4).
        ...
        The two gates come first and force 0 before any leg is scored:
          ...
          * **redundancy** — this body dies to our Knock Out this turn (`active_can_ko` for the
            Active, `_snipe_doomed_bench` for the bench), so we never spend a Hammer on a corpse.
        ...
        blank = {"relevance": 0.0, "relevance_energy": None, "relevance_attack_leg": 0.0,
                 "relevance_ability_leg": 0.0, "relevance_setback": 0}
        ...
        for j, eid in enumerate(energies):
            ...
            if best_j is None or got["relevance"] > best["relevance"]:
                best, best_j = {"relevance": got["relevance"], "relevance_energy": j,
                                "relevance_attack_leg": got["attack_leg"],
                                "relevance_ability_leg": got["ability_leg"],
                                "relevance_setback": got["setback_damage"]}, j
        return best

    def _line_attack_costs(self, card_id) -> tuple:
        """Every attack of every form in an opponent body's LINE, as
        ``([(damage, {EnergyType: slots}), …], ability_fuel_types)``.
        ...
        read here rather than re-derived so deny and the attach marginal (#139) cannot drift."""
        from collections import Counter
        if not self.stats:
            return (), frozenset()
        ids = {card_id} | set(self.combat.forward_card_ids(card_id))
        attacks, fuels = [], set()
        for cid in ids:
            st = self.stats.get(cid) if cid is not None else None
            if not st:
