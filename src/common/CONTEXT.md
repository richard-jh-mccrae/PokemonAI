...
the gate stack stay live pending their own swaps. **Built (ADR-0074, #186, 2026-07-27):** `deny_slot`
generalized to an opponent-target slot FAMILY — a `gust_target` kind (held gust-effect Trainer cards,
bench-only per `doctrine_gust.py`) keep-prices against the real per-body `opponent_target_value` /
`phase_scale` marginal instead of the flat disruption tier `deny` still uses; kill-switched
(`Pilot.gust_target_slots`, OFF by default — armed, `needs.SUPPLIES`'s `gust` tag routes to
`gust_target` INSTEAD of `deny`, never both). Snipe and the ADR-0044 forced-promotion pre-chip stay
OUTSIDE the DP entirely (neither is a held card competing for a slot) — both read the shared marginal
directly; #188 is where either is wired in as a live decider. Pure modules,
