...
HYPOTHESES = [
    ...
    Hypothesis(
        ...
        rationale="Lever A's favored half (ADR-0026 amendment) — the variance principle: unfavored "
                  ...
                  "targeted counterplay. Coverage-gated; board-dominated, never overrides a KO.\n\n"
                  "**Its two former neighbours are DELETED (ADR-0102), and this rung is now Lever A's "
                  "LAST rung-shaped consumer.** The old text sized the demotion against "
                  "`play-harlequin-vs-hand-size` (+25 → net +30) and called itself structurally "
                  "exclusive with `disrupt-when-unfavored` (≥0.55 vs ≤0.45); both are gone, so neither "
                  "arithmetic holds. What replaced them is not a rung on this axis at all — "
                  "`pilot._hand_size_relief_tactical` prices the SURVIVAL a refresh buys or costs, and "
                  "against a hand-size attacker it is the term that decides. The −15 survives on its "
                  "own axis (variance policy when FAVORED, ADR-0026), not as a counterweight to a "
                  "counterweight; if it ever needs re-sizing it is against the relief, not against a "
                  "flat that no longer exists.\n\n"
