...
`baseline_bench` / `baseline_tool` / `baseline_evolution` / `baseline_heal` / `baseline_opening` /
`baseline_sequencing` / `baseline_disruption`). The three card-archetype doctrines each own their
...
# The full deck-agnostic strategy = baseline clusters + the three doctrines (one flat scored list;
# ADR-0008/0025).
