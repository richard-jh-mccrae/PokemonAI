...
Sourced from the **Threat Clock**, not from `pilot._body_threat_rank` (ADR-0083 decision 3, and
...
`K = MAX_ATTACK_DAMAGE` is also the exit to the damage-scale `score` (ADR-0083 Amendment A1): since
...
#: adopted here by ADR-0083 Amendment A1, which corrects this ADR's original claim that
...
def ko_delta(turns_before: float | None, turns_after: float | None) -> float:
    ...
    The user's ruling (ADR-0083 decision 11): a chip is worth something only if it takes a TURN off
...
def target_relevance(*, incoming_damage: int = 0, turns_to_afford: int | None = None,
    ...
    Args:
        ...
        brief_priority: the matched MatchupPlan/Brief priority, signed and already scaled. A
            ...
            priority always applies. That asymmetry is load-bearing (ADR-0083 Amendment A3): a
