...
SUPPLIES: dict = {
    ...
    # ADR-0084 (Issue #197): a bench-drop tutor (Meowth ex's Last-Ditch Catch) fetches a SUPPORTER,
...
def _keep_slot_dp(slots, eligibility, resupply, exclude, capacity=None):
    ...
    ``capacity`` (ADR-0084, Issue #197) bounds how many cards may be assigned AT ONCE — the Bench
...
def deploy_marginal(slots, eligibility, resupply, index: int, *, capacity) -> float:
    ...
    points — the Deploy Marginal's assignment leg (ADR-0084, Issue #197, amendment E).
    ...
    * **zero on an empty Bench** — with slack capacity, dropping one unit costs the others nothing,
    ...
    scale (ADR-0084 amendment B) — this function must never be handed to a damage-scale consumer raw.
    """
