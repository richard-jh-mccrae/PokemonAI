    ...
    SoundRule(
        ...
        entry="authored constants inside firing equations (ROLE_TIER / TAG_TIER, readiness-leaf "
              ...
              "table is now derived from `ROLE_REGISTRY`, so the numbers have one home) — and, "
               ...
               "the guard, which left the then-current terminal band byte-identical. Issue #500 "
               "later adds the separately derived `ACTIVE_POSITION_MAX` to `POSITIONAL_MAX`; "
               "that bound is readiness plus maximum ranked survival exposure, not a fitted cap. "
               "Measured after the threat change: the guard bites on 45 of "
