...
def scale_count(gs: GameState, seat: int, var: str, energy_type: int | None = None) -> int:
    ...
    if var == "both_active_energy":
        return sum(len(provided_energy(gs, p.active)) for p in (b, ob) if p.active is not None)
    if var == "atk_bench_stage2":
        return len([p for p in b.bench if gs.stat(p.top).stage2])
    if var == "def_ex_in_play":  # their {ex} in play; a Mega Evolution Pokemon ex counts as an {ex}
        return len([p for p in gs.in_play(1 - seat)
                    if gs.stat(p.top).ex or gs.stat(p.top).megaEx])
    ...
    if var == "def_counters_all":               # EVERY body of theirs, not just the Active
...
def attack_damage(gs: GameState, attacker: PokemonInPlay, attack: Attack,
    ...
    if scale:
        ...
        elif scale["var"] == "both_in_play_named":  # both seats, SUBSTRING-matched: rules.md §9 makes an owner prefix PART of the name
            subs = scale["nameContains"]
            units = sum(1 for s in (seat, 1 - seat)
                        for p in gs.in_play(s)
                        if any(t in gs.stat(p.top).name for t in subs))
        elif scale["var"] == "milled_basic_energy":  # "for each Basic {W} Energy you discarded this way"
            from .schema import CardType
    ...
    for cb in cbs:
        if _cond_holds(gs, seat, cb["cond"], attack_cost=len(attack.energies), cb=cb):
            dmg += cb["n"]
    # riding the ATTACKER: its attacks do N less this turn, applied before W/R
    ...
    dstat = gs.stat(defender.top)
    # this-turn damage-bonus markers apply before W/R, opponent's Active only
    ...
    # Attached-tool attacker bonus, pre-W/R, opponent's Active only.
    if defender_is_active:
    ...
    # Stadium blanket, pre-W/R side: BOTH players' attackers, opposing Active only.
...
def apply_defender_mods(gs: GameState, attacker: PokemonInPlay,
    ...
    """Everything AFTER W/R. All pierced by an ignores-effects attack EXCEPT benched-Tera prevention."""
    ...
    if not ignore_effects:
        # ex-immunity (ADR-0032 prevent_ex panel) applies to BENCHED targets too, not just Active.
        ...
        # an on-the-Pokémon stadium blanket, which an ignores-effects attack pierces
