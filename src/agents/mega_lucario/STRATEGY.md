...
**Status:** Phase A **signed off** · Phase B build **in progress** (gated, tranched) · **Last grilled:** 2026-06-29 · **Author:** deck-genie + Richard
...
- [ ] Phase 6 Phase B build (gated): see §9 build plan
...
### Deferred to existing General-Strategy work
- **Damage-boost OHKO-line model** — Premium Power Pro **stacking** (+30/+60/+90) and Maximum Belt
  **+50-vs-ex breakpoints** (270→320 Dragapult). The General Strategy already defers this (needs a meta
  HP table + stacking model). Also verify **whether the engine counts a `megaEx` target as "{ex}"** for
  Maximum Belt's bonus.
- **Stadium matchup choice + opponent-board reads** — Gravity Mountain (default, no downside) vs
  Team Rocket's Watchtower (vs Colorless-ability decks; sequence after our Meowth); bump a harmful opp
  Stadium. Needs opp-board stage/type/ability reads (Read/Posture). v1: lean Gravity Mountain.
- **Meowth ex "bench for a needed Supporter" override** of `dont-bench-multiprize` — needs a
  "do I need a specific Supporter now" signal; situational, low priority.
...
- **T1 (foundation): ✅ DONE 2026-06-29 (gated).** `CardStat.recoil` (parsed in `_build_cache`; Wild
  Press→70 verified); Judge(1213) += `discard_hand` (Lunatone `draw` SKIPPED — would misfire
  `dig-before-commit` on the bench-play; Hariyama `gust` DEFERRED to T5 with its handling); deck
  `strategy.py` (roles/Line/params) + `main.py` (recoil wired); GENERAL `hold-wincon-dont-shuffle`
  (−25); DECK `fetch-the-engine-first` (+20). **Gates:** trigger tests 7/7
  (`tests/test_mega_lucario_triggers.py`); full suite 567 passed / 9 skipped; local Pilot smoke decides
  legally on mulligan/search/main (full self-match → CI; no `kaggle_environments` locally). _Diff
  presented; human commits._
- **T2:** discard-basic-F `Board` signal + `aura-jab-prefer-when-discard-fueled`; DECK
  `aura-jab-load-second-mega` (after identifying Aura Jab's attach SelectContext).
- **T3:** cooldown `Board` signal + `retreat-cooldowned-wincon-swap`.
- **T4:** GENERAL `prize-aware-attacker-choice` (conservative; never suppresses a lethal).
- **T5:** Heave-Ho four-mechanic gust split (hardest; ability-gust select + relaxed tempo gate).
- **Deferred:** damage-boost OHKO model; stadium opponent-board reads; Meowth "needed-Supporter" override.
