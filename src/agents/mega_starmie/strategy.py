...
below — Roles / Lines / params — so any deck making the same declarations inherits it):
    ...
    open-cinderace                  -> open-the-accelerator                  baseline_opening
                                       (SUPERSEDED 2026-07-28, ADR-0078: that rung was deleted
...
ROLES = {
    ...
    # (the `starter` Role on Cinderace + Staryu RETIRED 2026-07-28, ADR-0078 — it drove nothing
...
STRATEGY = Strategy(
    ...
    # deck's startable bodies (ADR-0078). Read by the general `open-the-declared-starter`.
