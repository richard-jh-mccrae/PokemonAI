...
- **Auto-subscribe on open, 5-minute check-in cadence.** As soon as a PR is opened in this repo,
  ...
  default ~1 hour. On each firing, check CI status:
  - **CI red:** pull in the failure details (merge conflicts or failed tests) and fix them, push
    the fix, then re-arm `send_later` for another 5-minute check-in.
