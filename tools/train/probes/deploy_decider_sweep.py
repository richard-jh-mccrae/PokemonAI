"""Deploy DECIDER sweep — the Decision Gate's input for the no-shadow swap (Issue #197, ADR-0086).
...
decider ranks (ADR-0086's `option_slot` extension). `correct` (the human label, where the frame has
...
#: The rungs the Deploy Marginal retires (ADR-0086). Forced to 0 in the NEW pilot so this probe
...
def _records_a_decline_it_cannot_state(rec, obs) -> bool:
    ...
    DORMANT since ADR-0086 decision 9 (2026-07-30), and kept anyway. The two frames it was built for
