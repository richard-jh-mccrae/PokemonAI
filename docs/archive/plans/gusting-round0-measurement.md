...
`test_gust_round0_corpus.py`. The one residual, **ep86091435 f119**, was ADJUDICATED in the
follow-up grill (2026-07-19): **refuted-by-better-line** in `reviewed.json` — the widened bench KO
oracle's 2-prize drag-and-spread line (gust a Duraludon → Phantom Dive KOs it AND the Relicanth)
supersedes the human's 1-prize development line, and is pinned as the ADJUDICATED corpus case.
Measured per `gusting-grill-spec.md` §Round 0. All 29 gust-adjacent
