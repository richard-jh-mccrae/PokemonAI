...
**The machine-checkable vocabulary of record is `src/common/card_tags.py`'s `TAG_REGISTRY`** (REQ-FUNC-0018), which also declares each tag's *consuming modules* and is walked against the shipped store by a test. This table is the narrative half — the derivation detail and the reasoning a registry entry cannot carry — and every tag in the registry has a row here.
...
| `supporter_tutor` | resource | A body whose **ON-PLAY Ability fetches a SUPPORTER** (Meowth ex's Last-Ditch Catch: bench-drop → search the deck for a Supporter). A *utility* body — it buys a Supporter and is then spent — which is why 1071 was re-modeled off `stall` onto this in 2026-07 (`_note_1071_stall_retired`). | Curated seed: an on-play Ability the probe reaches only as a generic `search`. |
| `tutor_pokemon` | resource | A fetch whose reachable class is **Pokémon** — the Planner's fetch-class read and the fetch doctrine's cost/benefit split on an Ultra Ball-shaped play. Note `fetch_closure` does NOT read it: ADR-0032's clause predicate replaced the tag-keyed `_FETCH_FILTERS` and only names the tag in prose. | Curated seed: the probe sees `DECK→HAND` without the class restriction. |
| `tutor_mega` | resource | A fetch restricted to a **Rule-Box Mega ex** — the narrowing `tutor_pokemon` cannot carry, so a wincon-in-hand read would otherwise over-claim on a generic Pokémon tutor. | Curated seed, as `tutor_pokemon`. |
| `tutor_trainer` | resource | A Supporter that searches **ANY Trainer card** out of the deck (Team Rocket's Petrel) — the Planner's route to a wincon-line Trainer. | Curated seed: the probe's deck cannot satisfy the search. |
| `cost_discard` | resource | A fetch that **COSTS a discard from hand** (the Ultra Ball shape). A blind, costly commitment the sequencer defers behind free development, and whose shed side `doctrine_fetch` prices. | Curated seed: the cost is a play-time choice, not a log shape. |
| `shuffle_hand` | disruption | A refresh that **shuffles BOTH hands away and redraws** (Iono, Unfair Stamp) — a *Gamble Line*, and ADR-0060 forbids reading it as a strip because it can REFILL a small opponent hand. | Curated seed: the probe sees the moves but not the symmetry. |
| `item_lock` | disruption | A body whose Ability **locks Item play**. Read on BOTH sides: it gates our own Item lines and prices their benched disruptor. | Curated: a passive Ability is never an option, so it is not probe-observable. |
| `prevent_ex_damage` | defensive | This body takes **NO damage from attacks by the opponent's Pokémon ex** (Crustle's Mysterious Rock Inn). The card fact that makes a 1-prize Stage 1 a deck's main attacker in an ex-dominated format. Read three ways — the damage oracle zeroes my ex's damage into it, `_body_threat_rank` boosts a line carrying it, and snipe relevance's `prevents_my_ex` leg. | Curated: passive prevention, text-derivable, not probe-observable. |
| `discard_energy_recur` | energy | A line that **reloads Basic Energy from a DISCARD pile** — the attrition read that stops a spent board being priced as spent. | Curated seed: the reload is an attack/Ability rider, not a play-event. |
| `tool` | structural-shaped | A **Pokémon Tool** — an attachment whose static modifiers ride the holder; `doctrine_tool` asks whether one is in hand at all before pricing an attach. ⚠️ **This is in tension with REQ-FUNC-0001** and the tension is INHERITED, not introduced: the tag has shipped on 1159 since before Issue #395, which only added 1174 Air Balloon so the store's existing instance became rebuild-reachable. It is not a second ruled exception like `team_rocket` — it is a live-consumed structural tag that predates the rule's current wording, recorded here rather than silently widened. Retiring it means moving `doctrine_tool` onto `CardStat`'s card type, which is a separate change. | Curated seed (`function_overrides.json`). |
...
| REQ-FUNC-0017 | **Rebuild-reachability** — `--fresh` is LOSSLESS (Issue #395). Every tag instance in the shipped table is re-derivable: either `classify_functions` emits it (`DERIVED_TAGS`, exported from the classifier and never re-transcribed) or `function_overrides.json` names it for that card. This is REQ-FUNC-0016's **other direction**, and its absence is what let eleven hand-authored instances sit in the table that `python tools/build_card_functions.py --fresh` deleted silently — `--fresh` sets `prior = {}`, and REQ-FUNC-0011's accumulate union was the only thing preserving them. One of the eleven was `prevent_ex_damage` on 345 Crustle, which three live consumers read. Also covers the store's FORMAT: the committed bytes must equal `build_card_functions.shipped_bytes`, so a rebuild is a real diff rather than a whole-file reformat, and the write is binary (LF on every OS — the store is `text=auto`, so the CI line-ending guard cannot see it; cf. ADR-0116). |
| REQ-FUNC-0018 | **Closed tag vocabulary** — `src/common/card_tags.py` (Issue #395). `TAG_REGISTRY` declares every legal tag with its source (`derived`/`curated`/`parametric`), a prose reason and its consuming modules; `PARAMETRIC_PREFIXES` declares the `name:N` families. `undeclared_tags` is the teeth, walked over the shipped store by `tag_vocabulary` rather than over a hand-kept list. Replaces nothing — `function_audit._CUES` is a 17-tag whitelist that *exempts* unknown vocabulary, so 25 of 42 shipped tags were audited by nothing at all. A tag declared with NO consumer is asserted to be read by nothing, so an "authored ahead of its consumer" label cannot go stale. |
