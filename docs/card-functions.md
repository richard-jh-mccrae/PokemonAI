...
| `rare_candy` | setup | A card that puts a **Stage 2 from hand straight onto its root Basic, skipping the Stage 1** (*"Choose 1 of your Basic Pokémon in play. If you have a Stage 2 card in your hand that evolves from that Pokémon, put that card onto the Basic Pokémon to evolve it, skipping the Stage 1"* — Rare Candy, id 1079). Read in two places, which is why it is a tag at all: the Turn Planner's Rare Candy KO line (`_is_rare_candy`) asks it of a PLAY option, and the backward playability oracle (`common.playability`, ADR-0103) asks whether one is reachable in hand or deck at all — a missing Stage 1 does not prove a Stage 2 dead. Distinct from `rush_evolve`: that one is a TUTOR that also evolves; this fetches nothing and needs the Stage 2 already in hand. | Curated seed (`function_overrides.json`): the evolve-skip is readable from card text, not probe-derived. |
