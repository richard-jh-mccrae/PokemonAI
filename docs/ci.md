...
| `tools/apply_seam_coverage.py` (the `strategy` filter) | `tests/strategy` | The POC-A2 apply-seam coverage census (Issue #269) is a `tools/` script whose only tests live in `tests/strategy`, because what it measures is `common/apply_option` + `common/snapshot_coverage`. Named explicitly so it does not fall through to the unmatched full-suite fail-safe |
...
`data/decider_lab/baseline.json` is currently pinned at **`31b1c28` (2026-07-31)**, **372 frames**
