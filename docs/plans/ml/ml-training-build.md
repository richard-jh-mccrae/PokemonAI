...
| S2b | WP2 | Eval harness | ☑ harness built 2026-07-19 (corpus-free) | design LOCKED → the s2b design (doc since deleted); `tools/sim/eval_{run,report,strata,aivat,spike}.py` + CLI + guide (`tools/sim/EVAL.md`); 32 tests green. **Pending corpus/WP1:** the spike's variance-reduction verdict (driver built) + the AIVAT fill (null seam) |
...
> **Design is LOCKED in the s2b design (doc since deleted)** (Fable design
...
> **Design is LOCKED in the s3a design (doc since deleted)** (Fable design
...
- **Remaining Fable 5 budget, if any, goes to design, not plumbing:** ✅ DONE 2026-07-13 —
  ...
  the s2b design (doc since deleted) locked; all three parallel-phase
