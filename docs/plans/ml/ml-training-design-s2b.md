# S2b Design — Eval Harness (WP2): GRILL SCOPE (not yet locked)

> **STATUS: GRILL SCOPE — decisions are OPEN.** This is the pre-grill agenda for the S2b design
...
> rewritten in place to LOCKED `D#` decisions and the [build ledger](ml-training-build.md) row S2b
...
[ml-training-build.md §S2b](ml-training-build.md) · evidence
...
- **Contestant resolution already handles both sources.** `tools/sim/battle.py:368` `resolve(spec,
  ...
  with `_git_short()` provenance. `--builds` defaults to the Build Ledger (`:408`). WP2's contestant
...
- **Neutrality gate exists.** `tools/sim/score_diff.py` `capture`/`diff`, modes `scores`
  ...
  and replay films. This is the tool G2/D-neutrality invariants lean on — WP2 reuses, doesn't rebuild.
- **Corpus films carry archetype per seat.** `tools/sim/corpus.py:234` `rec.replay(episode_id=eid,
  ...
  frames can stratify/label by true deck without re-deriving it — same fact S2a/S3b rely on.
...
job is to state precisely what WP2 is allowed to certify** (regression screen? adoption pre-filter for
G2? nothing without a stratum hit?) so we never re-import the invalid-signal mistake. This framing
...
- **Seed:** gate on Q1 — **cheat-agent stratum if duplicate-deal holds, else value-swing proxy**, and
  in EITHER case emit the stratum into C3 `strata` so G2 can read the skill-sensitive cell specifically
  (the local lesson says the *aggregate* cell is not trustworthy on its own).

### Q3 — AIVAT: how much now, how much stubbed until WP1?

C3 has an `aivat{variance_reduction,corrected_delta}|null` field; build plan says **stub the
interface now, fill after WP1** (the value net supplies the per-decision value estimates AIVAT needs;
...
- **Fork:** define the plug-in interface precisely now (input = per-decision value estimates over the
  ...
  G1 net exists — vs defer the interface entirely.
- **Seed:** **define + stub the interface this session** (it's a contract other tracks read), wire the
  computation post-G1. Low cost now, avoids a second C3 touch later. Explicitly NOT required for the
...
frozen-build opponent pool (the deferred-WP5 stand-in, catches regression + non-transitivity drift).
...
- **Fork:** (a) reuse `paired_ab` as-is by treating candidate/baseline as the two "configs" per fixed
  ...
  which is exactly the G2 use-case); vs (b) a new `eval*` emitter that generalizes to distinct
  contestant *agents* + an N-build checkpoint pool, with `paired_ab.paired_delta` reused only for the
  per-matchup CI math.
- **Seed:** **(b) new thin emitter, reusing `paired_delta` for the stats core.** The G2 primary
...
- **Fork:** what N (duplicate-dealt + stratified + AIVAT-corrected) detects a **2–3% winrate delta**
  at 95%? This sets the eval run's default `--n` and whether G2 is affordable at ~1000 games/min.
- **Seed:** derive it in-session from `paired_delta`'s variance formula (`Σ var_i / k²`) under the
  ...
  1000 games/min so G2's cost is known before S3b lands.
...
1. Fork-replay spike **go/no-go** + the exact capture spec (Q1) — and the C3 `strata` behavior in
   both branches.
2. The stratification method chosen (Q2) and what stratum G2 is permitted to read.
3. The AIVAT interface signature + now-vs-post-G1 split (Q3).
4. The emitter shape + which existing stats code it reuses (Q4).
5. The default eval N + its wall-clock, tied to the G2 affordability check (Q5).
6. A one-paragraph **certification charter**: exactly what a WP2 report is allowed to claim, closing
   the "gauntlet invalid vs gauntlet done right" tension so G2 can't re-import the invalid signal.

## Non-goals (out of scope for the S2b grill)

- League / exploiter probe (WP5, deferred until WP4 ships checkpoints) — WP2's checkpoint pool is the
  interim stand-in, not the league.
- The value net itself (WP1/S2a) and expert-iteration (WP4/S3b) — S2b measures them, doesn't build them.
