...
- **Gust repoint (old Issue #189): target side DONE 2026-08-02 (ADR-0107, carved into
...
| `prize-lethality` | `state_value._predicted_loss` **case 1** (`LOSS_PRIZES` prize lethality, ADR-0064 Amendment B) | `structural` | a body whose Knock Out gives them their **last prize** — `docs/rules.md` §7 case 1, a DIFFERENT fact from case 2 guarded by the same term and the same clock. On the terminal term because the fact is the PRODUCT of a body's prize yield and their remaining count, which the double-counting rule forbids `survival` and `prize_race` to form between them (Issue #283; the argument lives in ADR-0064 Amendment B). **Binary** for the POC | — |
...
| `composer-budget-caps` | `composer.BEAM_WIDTH` / `SEQUENCE_DEPTH` / `EPSILON` (+ `board_expectation.BRANCH_CAP`, declared here rather than three times) | `authored-scaffold` | how much of the sequence tree ONE decision may explore. Owed by Issue #385, which builds the composer and is therefore the first issue for which a budget cap exists to be spent — `board_expectation`'s header and `board_choice`'s both state the debt and name this entry. **Three usages, the third a REUSE:** the beam's own width/depth/epsilon; `BRANCH_CAP` = 12 as the expectation-branch cap on a **chance** node; and that *same* `BRANCH_CAP` read by `board_choice.deferred_target` as the Target-Ranker top-*m* on a **choice** node — both cost leaf evaluations on one option, so a second constant would be the drift the whitelist exists to prevent. Derived from post-OEC menu width P50 6 / P95 12 / max 23 and leaf P95 6.57 ms against the grader's ≥ 3.0 s per-decision floor: `(1 + 3 × 4) × 12 = 156` leaves ≈ 1.03 s, ~34% of it. **Then measured end to end** on an idle box (`tools/train/composer_lab.py`): per-decision median 24.8–26.5 ms, P95 0.40–0.43 s, max 1.31–1.40 s — the max at ~44–47% of the floor, so the bound holds. The measurement corrected the bound's FORM: it is quoted at the P95 width, so it bounds the P95 decision and not the max one — corpus leaf evals are 3515 and the widest frame costs 323 against the 156 that form predicts, while the same formula at the observed max width gets it right (`(1+3×4) × 27 = 351`). The cost is **leaf-bound, not beam-bound**, and re-sizing was measured (k×depth over {2,3,4}² does not move the corpus max — leaf evals saturate at depth 2). The width is a TIME budget, not an acceptance-COVERAGE one: ruled first steps land at rank P50 2 / P90 6 / P95 7, so k=4 reaches 81.7% of them, and acceptance frame f82 sits at rank 5 — reported, not widened around (§S12.2). ⚠️ Wall-clock is only meaningful on an IDLE machine: a first pass taken under a running pytest read max 2.45–4.05 s and raised a false crosses-the-floor alarm; the leaf-eval count is identical under load, so cross-check against it. Corpus figures and machine live on `composer.BEAM_WIDTH`. `EPSILON` is not authored at all: it is `family_diag.DECIDER_FLOOR`, **swept** rather than assumed | re-measured on GRADER hardware post-POC (Issue #273); the derived P95 it rests on is a dev-machine LOWER BOUND on the leaf half alone |
