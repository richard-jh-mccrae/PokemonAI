...
| `composer-budget-caps` | `composer.BEAM_WIDTH` / `SEQUENCE_DEPTH` / `EPSILON` (+ `board_expectation.BRANCH_CAP`, declared here rather than twice) | `authored-scaffold` | how much of the sequence tree ONE decision may explore. Owed by Issue #385, which arms the composer and is therefore the first issue for which a budget cap is live — `board_expectation`'s header states the debt and names this entry. Derived from post-OEC menu width P50 6 / P95 12 / max 23 and leaf P95 4.46 ms against the grader's ≥ 3.0 s per-decision floor: `(1 + 3 × 4) × 12 = 156` leaves ≈ 0.70 s, ~23% of it. **Then measured end to end** (`tools/train/composer_lab.py`, 371 frames): per-decision median 10.0 ms / P95 152 ms / max 684 ms — the bound held. `EPSILON` is not authored at all: it is `family_diag.DECIDER_FLOOR`, **swept** over {0.0 … 0.05} with nothing moving, so it sits inside the flat region at the measured noise floor | re-measured on GRADER hardware post-POC (Issue #273); the derived P95 it rests on is a dev-machine LOWER BOUND on the leaf half alone |
