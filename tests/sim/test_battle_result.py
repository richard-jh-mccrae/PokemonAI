...
def test_aggregate_header_recomputes_from_matches_and_keeps_a_seat():
    ...
    assert r["win_rate"] == pytest.approx(2 / 4)            # derived from same matches
    assert [m["a_seat"] for m in r["matches"]] == [0, 1, 0, 1]   # per-Match source of truth, a_seat kept
    assert "verdict" not in r                               # interpretation derived on read, never stored
...
def test_battle_result_round_trips_through_the_committed_log(tmp_path):
    ...
    assert r2["battle_id"] == 2            # monotonic battle_id derived from log
...
def test_contestants_carry_provenance_overlay_and_deck_hash():
    ...
    assert a["kind"] == "build"                          # has an artifact -> Build (not working-tree)
    assert a["overlay"] == {"overrides": {"x": 5}} and b["overlay"] is None   # config under test
