...
* a card gains Effect Clauses with no `CLAUSE_JUDGEMENT` ruling — the census would then have to
  guess whether the clauses cover the whole card, which is the one column it must never fabricate;
...
def census():
    mod = _load()
    cards, effects = mod.load_cards(), mod.load_effects()
...
def test_every_clause_bearing_pool_card_has_a_ruling(census):
    ...
    mod, cards, effects, pool = census
    assert mod.validate(pool, cards, effects) == []
...
def test_sites_resolve_to_declared_fates_only(census):
    ...
    mod, cards, effects, pool = census
    sites, _aside = mod.census(pool, cards, effects)
...
def test_report_block_is_present_and_regenerable(census, tmp_path):
    ...
    mod, _cards, _effects, _pool = census
