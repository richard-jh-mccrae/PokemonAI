...
def _poke(cid, *, hp, energies=(), energy_cards=None, serial=1, damage=0):
    ...
    Special Energy must go through `energy_cards=`, a sequence of ``(card_id, units)`` pairs:
    Ignition Energy is card 17 providing ``(0, 0, 0)``, Rock Fighting is card 20 providing ``(6,)``
    (Issue #297)."""
    if energy_cards is None:
        energy_cards = [(c, (c,)) for c in energies]
    return {"id": cid, "hp": hp - damage, "serial": serial,
            "energies": [u for _, units in energy_cards for u in units],
            "energyCards": [{"id": c, "serial": 0} for c, _ in energy_cards if c]}
