...
def test_chain_value_is_the_discounted_closure_max(pilot, board):
    """chain_value(Petrel) = δ² × grab_value(Solrock) exactly. Pinning the EXACT number also pins the
    exclusions: the held Mega Lucario ex must NOT be the max, and it is MAX not SUM."""
...
def test_chain_is_zero_for_a_non_tutor(pilot, board):
    """A card with no deck FETCH clause prices chain 0 — the endorser fail direction (unknown chain →
    0, never inflates)."""
...
def test_supporter_chain_dies_with_the_slot_spent(pilot, board):
    """A Supporter tutor with the one-per-turn Supporter slot already spent prices chain 0 — the chain
    is not free this turn (fail-closed)."""
...
def test_chain_decays_once_the_end_target_is_acquired(pilot, board):
    """With Solrock (virtually) in play the chain's best end value collapses to noise and the
    discounted chain drops below the opener floor — the rung goes silent on a re-score."""
...
def _hop_obs(base: dict, pulled: tuple, option_pred, pilot) -> dict:
    ...
    pulled card moves deck → discard, the Supporter slot is spent, options are the clause targets."""
...
def test_hop_three_gong_select_takes_the_solrock(pilot, rec):
    """Hop 3: Fighting Gong's select is on the menu and Solrock (+22) wins over the line piece (+18),
    the {F} Energy (+3) and the redundant engine pieces — the chain lands on its END target."""
    is_trainer = lambda st: st is not None and not st.is_pokemon and not st.is_energy
...
def _petrel_select(rec, pilot, *, makuhita_in_hand=False, poke_pads_in_pool=None):
    """Petrel's Trainer select (hop 2), optionally with Makuhita in hand and the pool thinned to
    ``poke_pads_in_pool`` Poké Pad copies (None = all)."""
    is_trainer = lambda st: st is not None and not st.is_pokemon and not st.is_energy
    obs = _hop_obs(rec.obs, (PETREL, 55), is_trainer, pilot)
...
def test_no_wanted_evolution_no_preservation(pilot, rec):
    """Without a wanted evolution even the LAST Poké Pad is spent freely — the rung is need-gated, not
    a blanket Poké-Pad floor."""
...
def test_chain_graph_is_full_scope_and_cycle_safe(pilot, board):
    """Petrel's `trainer` clause reaches the deck's Trainers, including Petrel ITSELF (the cycle the
    path-`seen` set cuts). Never a Pokémon or an Energy."""
