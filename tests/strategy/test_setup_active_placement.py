"""The pregame Set-Up ACTIVE pick: one rule reading one deck declaration (ADR-0077).
...
change. The two seams (ADR-0077 / the /to-spec seam sketch):
...
def _authored_agents() -> list[str]:
    """Agents with a doctrine — a `STRATEGY.md` is the marker deck-genie has been run (ADR-0077
...
def test_f2_opens_the_utility_body_over_the_fragile_line_base():
    ...
    on an equation that never runs on this path — ADR-0070 §4 re-ruled it out of scope and ADR-0077
...
def test_ml_f1_opens_the_attacker_over_the_draw_engine():
    ...
    `card_id == SOLROCK`, the exact card-id reflex ADR-0077 removes. Order-independent."""
...
def test_the_highest_ranked_body_PRESENT_wins_not_merely_rank_one():
    ...
    equivalent to a rank scale under a COMPLETE list (ADR-0077 decision 5)."""
    pilot = _pilot("dragapult_ex")
...
def test_every_authored_agent_declares_a_starter_priority():
    """ADR-0077 decision 5, the presence half. With the five old rungs deleted, a deck that declares
...
def test_every_declaration_ranks_every_startable_body_in_the_deck():
    """ADR-0077 decision 5, the completeness half — the invariant that makes the single-winner read
...
def test_the_exempt_agents_are_exactly_the_pre_doctrine_ones():
    """The exemption is asserted EXPLICITLY (ADR-0077 amendment A), so adding an agent cannot
