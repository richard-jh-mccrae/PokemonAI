...
DISCARD_SEL = 8         # SelectContext.DISCARD (NB `pilot_helpers.DISCARD` is the AreaType)
WINCON = 900            # Mega-ex win-condition
PREEVO = 800            # its pre-evolution
WATER = 3               # reusable Basic Energy
IGNITION = 17           # discard-at-EOT burst energy (CCC on an Evolution)
CRUSH = 1120            # Crushing Hammer — `energy_denial`
OPP = 678               # energized opponent attacker (Mega Lucario ex shape)
POFFIN = 1086           # Buddy-Buddy Poffin — bench-fill
...
ITEM_CT, BASIC_ENERGY_CT, SPECIAL_ENERGY_CT = 1, 5, 6      # CardType codes, src/cg/api.py
...
def _sequenced(pilot, obs, decision=None):
    """Pass ``decision`` when the caller also asserts on `deferred`: the sequencer MARKS that flag, so a
    second `explain` marks a second set of traces and leaves the caller's copies untouched."""
    select = obs["select"]
...
def _stats(attacks=None):
    """`cardType` is LOAD-BEARING: `board_delta` routes an ATTACH on `stat.is_energy`, so an omitted type
    makes the seam refuse the option — a fixture bug that reads exactly like a coverage ceiling."""
...
FIGHTING = 6      # EnergyType.FIGHTING — the TYPE code
F_ENERGY = 6      # Basic {F} Energy — the CARD ID
...
def test_build_active_wincon_stands_down_for_a_discard_energy_when_the_cheap_attack_kos():
    """Don't burn a finite discard-EOT Energy toward the big attack when the cheap attack already KOs.
    Scoped to `discard_eot`: a reusable Water still builds toward the payoff."""
    pilot = _pilot()
    ign = opt(ATTACH, area=HAND, index=1, inPlayArea=ACTIVE, inPlayIndex=0)   # attach Ignition
...
def test_keep_key_cards_at_discard_protects_the_burst_energy():
    ...
    # The `_DISCARD` ladder is deleted; the keep-value equation protects the burst on its own.
...
def test_attach_lethal_unlocks_a_ko_via_ignition_ccc():
    pilot = _pilot()
    # Jetting Blow (120) can't KO a 200-HP Active; Ignition (CCC on an Evolution) unlocks Nebula 210.
    attach_ign = opt(ATTACH, area=HAND, index=0, inPlayArea=ACTIVE, inPlayIndex=0)   # Ignition
    attach_w = opt(ATTACH, area=HAND, index=1, inPlayArea=ACTIVE, inPlayIndex=0)     # Water
...
def _denial_pilot(**kw):
    """Knows Crushing Hammer (`energy_denial`) plus the win-con line."""
    ...
    # Deny Relevance is TYPED: without BOTH the attack's `energyTypes` and a Provider-resolvable
    # Energy card, the strip reads relevance 0 and the sequencing claims below test nothing.
...
def test_play_energy_denial_sequences_the_strip_before_a_higher_value_attack():
    """`_finish_turn_last` sequences the free Item ahead of the turn-ending attack, so the agent
    disrupts AND still attacks. The opponent sits at exactly 1 Energy so the strip denies 130 -> 0."""
    pilot = _denial_pilot()
    ...
    assert traces.options[0].score > 0
    assert traces.options[1].tactical > traces.options[0].score
...
def test_play_energy_denial_fires_in_setup_against_a_developing_attacker():
    """Being in SETUP does not suppress the strip."""
    pilot = _denial_pilot()
...
def test_energy_denial_stands_down_when_you_can_already_ko_the_active():
    ...
    assert "play-energy-denial" not in _fired(traces.options[0])
    assert pilot.decide(obs) == [1]


@pytest.mark.req("REQ-GEN-0031")
def test_energy_denial_stands_down_on_a_best_affordable_ko_not_just_the_cheapest():
    """The stand-down keys on `active_can_ko` (BEST affordable attack), not `active_cheap_attack_kos`."""
    pilot = _denial_pilot()
    ...
    assert board.opp_active_can_damage_us                 # not the harmless-strip case
...
def _search_pilot(deck):
    """PREEVO is the only Basic in `deck`; WINCON the only Mega ex."""
...
def _ctx(pilot, obs, i):
    """The per-option Context the Pilot scores."""
    select = obs["select"]
...
def test_deck_definitely_empty_of_is_sound_only_when_all_copies_are_accounted():
    ...
    # All 3 seen OUTSIDE the deck -> PROVABLY empty; hidden prizes cannot hold a 4th copy.
    ...
    # Only 2 of 3 seen; the 3rd could be in the 6 hidden prizes -> NOT certain.
...
def test_deck_empty_counts_revealed_prizes_and_evolution_stacks():
    ...
    # A FACE-UP prize is out of the deck: Active + discard + revealed prize = 3 -> empty.
...
def test_dont_search_stands_down_a_bench_fill_whose_only_target_is_gone():
    ...
    # One Staryu unaccounted -> rule stays silent.
...
def test_dont_search_stands_down_mega_signal_when_all_megas_are_gone():
    ...
    # One Mega unaccounted -> NOT certain -> rule stays silent.
...
def test_flexible_tutor_is_not_redundant_when_the_wincon_is_in_hand():
    # Ultra Ball fetches ANY Pokémon, so a held wincon does NOT make it redundant.
    pilot = _search_pilot([WINCON] * 3 + [PREEVO] * 3 + [FILLER] * 54)
...
def _endorse_ultra_pilot():
    """Endorses Ultra Ball hard, so only the `cost_discard` commitment tiering decides its sequence."""
...
# `test_dig_before_commit_skips_a_cost_discard_search` was DELETED with its rung (Issue #386); the
# successor is `test_cost_discard_search_is_sequenced_after_a_free_dig_...`, which asserts the ORDER.


@pytest.mark.req("REQ-GEN-0033")
def test_cost_discard_search_is_sequenced_after_a_free_dig_even_when_it_scores_higher():
    """A discard-cost play is a COMMITMENT (tier 2), so the FREE dig goes first despite scoring lower.
    The two FILLERs are load-bearing: Ultra Ball's *"discard 2 other cards"* is illegal without them."""
    ...
    traces = pilot.explain(obs).options
    assert traces[0].score > traces[1].score
...
def test_attach_beats_a_cost_discard_search_the_ep82228640_fr7_shape():
    pilot = _search_pilot([WINCON] * 3 + [PREEVO] * 3 + [FILLER] * 54)
    attach = opt(ATTACH, area=HAND, index=1, inPlayArea=ACTIVE, inPlayIndex=0)   # W -> Staryu
    play_ball = opt(PLAY, area=HAND, index=2)                                    # Ultra Ball
    # Two FILLERs so Ultra Ball's "discard 2 OTHER cards" is payable.
...
# ═══ The single-pick MAIN decision belongs to `common.composer` (Issue #386), so this file asserts
# what it owns: the ladder's own RANKING (`_ranked`) or ORDER (`_sequenced`).
...
def test_the_composer_ties_these_boards_and_why():
    """A board built for ONE rung is too sparse to difference: `state_value` reads the END-OF-TURN board,
    and what makes the Active right lives in `terminal_ev`, which needs an attack ON THE MENU."""
    pilot = _pilot()
...
def test_the_energy_denial_doctrine_is_a_composer_COVERAGE_CEILING_not_a_disagreement():
    """The Hammer is REFUSED as a beam candidate, not priced at zero — indistinguishable from the
    outside, which is why the gap text is asserted (Issue #383 territory, not a ruling)."""
    pilot = _denial_pilot()
    ...
    # Positive control: the instrument must be able to report NO gap.
