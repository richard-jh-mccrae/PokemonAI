...
def test_the_active_preference_prior_cannot_override_a_build_step(steps):
    ...
    zero-build Active beat a body that gains real progress."""
...
def test_the_shipped_tuned_layer_respects_the_bands(steps, hid, band):
    """tuned.json wins over the authored seed (ADR-0018), so a Tuner re-fit can reintroduce the weight
    coincidence unless the band is asserted on the weight each agent DECIDES with."""
...
def test_the_disruptor_lock_weight_beats_a_bench_recipient_by_design_and_no_more(steps):
    """The maneuver spends the Energy on the enabling retreat, so its rung clears the biggest build step
    a bench recipient could offer — and stops there, staying structural rather than a new value term."""
...
def test_only_the_three_structure_rungs_survive():
    """ADR-0069 §7: the other rungs are DELETED, not suppressed, so no dead weight re-enters tuning."""
