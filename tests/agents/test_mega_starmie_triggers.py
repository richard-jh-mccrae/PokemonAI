...
# Hero's Cape's +100 is no longer hardcoded in the deck: the Tool Doctrine (ADR-0028) reads it off the
# parsed `CardStat.hpBonus` (= 100) and deploys PROACTIVELY by survival-turns board-math (no longer the
# reactive breakpoint). These confirm the wiring fires through the real mega_starmie Pilot — ACE SPEC
# Cape, real deck roles (Mega Starmie ex = win_condition), the weakness-aware `Board.incoming_active_damage`.
...
_CAPE_STATS = DictCardStatProvider({
    ...
    8888: CardStat(8888, energyType=FIRE, maxDamage=400),       # opp hits 400: +100 (430) buys a survival turn
    7777: CardStat(7777, energyType=FIRE, maxDamage=500),       # opp hits 500: doomed even at 430 -> wasted
    6666: CardStat(6666, energyType=FIRE, maxDamage=200),       # opp hits 200: +100 still banks a survival turn
})
