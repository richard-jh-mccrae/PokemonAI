...
from common.scouting.briefs import (
    Brief, load_briefs, match_brief, resolve_brief_cards,
)
...
def test_generic_support_pokemon_roles_preserve_their_purpose(name, purpose):
    ...
    roles = general_pokemon_roles(ids, provider)
    assert ids and all(
...
def test_every_shipped_brief_pokemon_resolves_to_a_role():
    ...
    for brief in load_briefs():
        _, resolved = resolve_brief_cards(
            ...
            forward_ids=provider.forward_card_ids)
        for entry in brief.pokemon:
            ...
            assert all(resolved[card_id] in POKEMON_ROLES for card_id in ids), (
                f"{brief.slug}: {entry['card']} has an unknown Role")
