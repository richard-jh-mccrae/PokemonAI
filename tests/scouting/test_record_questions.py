...
def test_prize_value_ladder_mega_ex_plain():
    ...
    assert CardStat(666, ex=True).prize_value == 2
...
def test_is_ex_body_covers_ex_and_mega_ex():
    ...
    assert CardStat(666, ex=True).is_ex_body
...
def test_card_kind_predicates_key_on_the_cardtype_enum():
    ...
    assert CardStat(1, cardType=1).is_item
    assert CardStat(2, cardType=2).is_tool
    assert CardStat(3, cardType=3).is_supporter
    assert CardStat(5, cardType=5).is_basic_energy
    assert not CardStat(6, cardType=6).is_basic_energy
    assert CardStat(5, cardType=5).is_energy
    assert CardStat(6, cardType=6).is_energy       # Special Energy is still an Energy
    assert not CardStat(3, cardType=3).is_energy
...
def test_is_typed_basic_energy_needs_both_kind_and_type():
    ...
    assert not CardStat(3, cardType=5, energyType=None).is_typed_basic_energy
    assert not CardStat(3, cardType=6, energyType=3).is_typed_basic_energy   # Special never counts
...
def test_can_pay_cheapest_is_the_fail_closed_affordability_idiom():
    assert CardStat(678, minAttackCost=2).can_pay_cheapest(2)
    assert not CardStat(678, minAttackCost=2).can_pay_cheapest(1)
    # Unknown cheapest cost fails CLOSED — a my-side affordability claim is never assumed.
    assert not CardStat(678, minAttackCost=None).can_pay_cheapest(4)
    ...
    assert not CardStat(678, minAttackCost=0).can_pay_cheapest(4)
