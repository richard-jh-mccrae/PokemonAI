...
def test_structural_facts_are_not_tagged():
    # ex/Mega-ex, trainer subtype, and ACE SPEC are read straight off CardData at runtime, so the
    # function table is purely *behavioral* — a card with no behavioral probe gets no tags.
...
def test_probe_actor_draw_is_draw_tag():
    ...
    # the *opponent* drawing (e.g. forced by my card) is not "I draw"
...
def test_probe_deck_to_hand_move_is_search():
    # MOVE_CARD(6) DECK(1) -> HAND(2) by the actor = tutor
...
def test_probe_deck_look_is_dig():
    # MOVE(6) DECK(1)->LOOK(12) by the actor = looked at the top of the deck (Roto-Stick)
...
def test_probe_look_to_hand_is_draw_not_search():
    # "look at the top N, put 1 into your hand" (Drakloak's Recon Directive) is a *draw* engine,
    # not a deck-wide tutor — only a direct DECK->HAND move is `search`.
...
def test_probe_deck_to_bench_is_search():
    # tutoring a Basic from deck into play (Precious Trolley) is a search
...
def test_probe_own_switch_is_switch():
    # SWITCH(8) on the actor's side = moved my own Active out (Switch item)
...
def test_probe_opponent_switch_is_gust():
    # SWITCH(8) on the *opponent's* side = forced their Active to change (Boss's Orders)
...
def test_probe_attach_by_nontool_is_energy_accel():
    # ATTACH(11) by the actor from a non-Tool card = put Energy into play (Energy Coin / Waitress)
    p = {"actor": 0, "logs": [{"type": 11, "playerIndex": 0}], "contexts": []}
    assert "energy_accel" in classify_functions({"category": "item"}, probe=p)
    # a Tool attaching *itself* must NOT count as acceleration
...
def test_probe_each_special_condition_maps_to_its_own_tag():
    ...
    # recovering from a condition is not inflicting it
...
def test_probe_opponent_hand_to_deck_is_hand_disruption():
    # MOVE_CARD_REVERSE(7) opp HAND(2)->DECK(1) = forced the opponent to shuffle their hand (Iono/Judge)
    ...
    # MY OWN hand reshuffling (Judge resets me too) is not disruption *of the opponent*
...
def test_probe_opponent_energy_to_discard_is_energy_denial():
    # MOVE_CARD(6) opp ENERGY(8)->DISCARD(3) = knocked an Energy off the opponent (Crushing Hammer)
    ...
    # discarding MY OWN Energy (an attack cost) is not denial
...
def test_probe_discard_to_hand_is_recycle():
    ...
    # DECK->HAND is a search, not a recycle (different origin)
...
def test_build_table_merges_probe_and_omits_untagged():
    ...
    assert table[1] == ["draw"]     # behavioral only (the supporter category is *not* a tag)
    assert 2 not in table           # no behavioral tags → omitted from the shipped table


@pytest.mark.req("REQ-FUNC-0006")
def test_build_table_applies_overrides():
    table = build_function_table({5: {"category": "item"}}, probes={}, overrides={5: ["gust"]})
    assert table[5] == ["gust"]     # override only (the item category is not a tag)
...
def test_accumulate_unions_prior_run_tags():
    ...
    assert acc[2] == ["status"]                  # a card seen only this run is kept
    assert acc[3] == ["recycle"]                 # a prior-only card is NOT dropped (stochastic miss)


@pytest.mark.req("REQ-FUNC-0011")
def test_accumulate_never_removes_a_once_observed_tag():
    # this run's probe missed the (rng-gated) tag, but a prior run caught it → it survives
