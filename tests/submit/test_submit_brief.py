...
def test_manifest_includes_general_strategy_hypotheses_with_overlay(tmp_path):
    agent = _agent_with_tuned(tmp_path, {"dig-before-commit": 77.0})
    manifest = build_manifest(agent)

    gen = manifest["general_strategy"]["hypotheses"]
    hyp = next(h for h in gen if h["id"] == "dig-before-commit")  # shared General Strategy rule
...
def test_manifest_effective_weight_layers_authored_override_under_tuned(tmp_path):
    ...
    (agent / "strategy.py").write_text(
        ...
        "{'dig-before-commit': 33.0, 'open-the-declared-starter': 44.0})\n", encoding="utf-8")
    ...
    assert gen["dig-before-commit"]["effective"] == 33.0           # authored seed override applies
    assert gen["dig-before-commit"]["seed_override"] == 33.0
